...
configure_terminal() {
    ...
    curl --create-dirs -LO --output-dir ~/.config/alacritty/catppuccin https://github.com/catppuccin/alacritty/raw/main/catppuccin-macchiato.toml

    ln -sf $DOTFILES_DIR/alacritty/alacritty.toml ~/.config/alacritty/alacritty.toml

    echo "Linking nvim config"
    mkdir -p ~/.config/nvim
    ln -sf $DOTFILES_DIR/nvim/init.vim ~/.config/nvim

    ln -sf $DOTFILES_DIR/nvim/vimrc ~/.vimrc
...
if ask "Configure the terminal (tmux, alactitty, nvim)?" Y; then
    configure_terminal
fi
...
install_java() {
    ...
    # latest v18
    JAVA_VERSIONS=("$(asdf list all java | grep zulu-18 | tail -1)")
